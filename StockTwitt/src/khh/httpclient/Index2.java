...
import khh.httpclient.twitter.TimeLine2;
...
//		Thread.sleep(10000);
		...
		for (int i = 0; i < workCnt; i++) {
			TimeLine2 t = new TimeLine2(queue);
			worklist.add(t);
			Thread.sleep(100);
		}
				...
				while(true){
					try {
						for (int i = 0; i < worklist.size(); i++) {
							...
							log.info("ThreadMonitor! ("+t.getName()+"  :  "+i+")("+worklist.size()+") stste "+t.getState()+ " queue:"+queue.size());
