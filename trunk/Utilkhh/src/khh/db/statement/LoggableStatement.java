...
// PreparedStatement  pstmt = new LoggableStatement(conn, sql.toString());
//pstmt.setString(index++, param.getString("sched_cycle"));
//pstmt.setString(index++, param.getString("sched_ishour"));
//pstmt.setString(index++, param.getString("sched_stime"));
//pstmt.setString(index++, param.getString("sched_etime"));
...
public class LoggableStatement implements PreparedStatement {
	 ...
	 *
	 * @param Connection
	 *            java.sql.Connection a JDBC-connection to be used for
	 ...
	 * object. After a call to this method, the method <code>getWarnings</code>
	 * will return null until a new warning is reported for this Statement.
	 *
	 * @exception SQLException
	 *                if a database access error occurs
	 */
	 ...
	 * JDBC 2.0
	 *
	 * Submits a batch of commands to the database for execution. This method
	 * is optional.
	 *
	 ...
	 *
	 * <P>
	 * <B>Note:</B> If you are processing a ResultSet, any warnings
	...
	/**
	 * Sets the designated parameter to a java.lang.BigDecimal value. The
	 * driver converts this to an SQL NUMERIC value when it sends it to the
	 * database.
	 *
	 * @param parameterIndex
	 *            the first parameter is 1, the second is 2, ...
	 ...
	 * call its method <code>writeSQL</code> to write it to the SQL data
	 * stream. If, on the other hand, the object is of a class implementing
	 * Ref, Blob, Clob, Struct, or Array, then the driver should pass it to the
	 * database as a value of the corresponding SQL type.
	 *
	 * This method throws an exception if there is an ambiguity, for example,
	 * if the object is of a class implementing more than one of those
	 * interfaces.
	 *
	 * @param parameterIndex
	 *            the first parameter is 1, the second is 2, ...
	 ...
	 * the JDBC driver should call its method <code>writeSQL</code> to write
	 * it to the SQL data stream. If, on the other hand, the object is of a
	 * class implementing Ref, Blob, Clob, Struct, or Array, the driver should
	 ...
	 * argument's size relative to the driver's limits on VARCHARs) when it
	 * sends it to the database.
	 *
	 * @param parameterIndex
	 *            the first parameter is 1, the second is 2, ...
	 ...
	 * {@link java.sql.PreparedStatement PreparedStatement}that this <code>LoggableStatement</code>
	 * is a wrapper for.
	 * <p>
	 *
